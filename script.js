...
mm.add("(min-width: 800px)", () => {
  ...
  // BENEFITS TO PRODUCTS (DESKTOP: NORMAL SCROLL)
  ScrollTrigger.create({
    trigger: "#products",
    start: "top center",
    end: "center center",
...
mm.add("(max-width: 799px)", () => {
  ...
  ScrollTrigger.create({
    ...
    onEnter: () => {
      gsap.to(".bottle", {
        scale: 0.9, // Change: 1.2 hataavi ne 0.9 karyu (Zoom bandh karva)
        y: -30, // Change: -150 ni jagya e -30 karyu (Jethi position stable rahe)
        x: 0,
        rotate: 0,
        duration: 1.5,
        ease: "power3.out",
  ...
  // BENEFITS TO PRODUCTS (MOBILE: SHRINK FINISHES FAST)
  ScrollTrigger.create({
    trigger: "#products",
    start: "top center",
    end: "+=10", // <--- Animation 1 Finishes here
    scrub: 1,
    onEnter: () => {
      gsap.to(".bottle", {
        scale: 0.6,
  ...
  // HIDE BOTTLE (MOBILE: FADE STARTS IMMEDIATELY AFTER SHRINK)
  ScrollTrigger.create({
    trigger: "#product-section",
    // 👇 CHANGED: "top center+=150" means "Start fading exactly when the shrink animation ends"
    // If this is still too late, change "+=150" to "+=0" or just "top center"
    start: "top center",
    onEnter: () => {
      gsap.to(".bottle", {
        opacity: 0,
...
var content = {
  "t-and-c": {
    ...
    html: `
            ...
            <p>Spill N Fill shall not be liable for any direct, indirect, incidental, or consequential damages resulting from the use of our website, products, or services.</p>
...
if (linkDisclaimer)
  linkDisclaimer.onclick = function (e) {
    e.preventDefault();
